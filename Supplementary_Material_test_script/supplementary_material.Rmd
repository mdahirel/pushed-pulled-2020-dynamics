...
We found that released wasps did lay their eggs closer from release sites in landscapes with restricted connectedness compared to controls (**Supplementary Figure S3.1**). In addition, we confirmed higher fecundity led to higher egg-laying distances (see model summary in linked code).
...
mod_front %>%
  ...
  mutate(treatment=ifelse(str_detect(name,"control"),"standard", name)) %>%
...
mod_front %>%
  ...
  mean_hdi() %>% 
  mutate(entry=paste(round(value,2)," [",round(.lower,2),", ",round(.upper,2),"]",sep="")) %>% 
  mutate(treatment=ifelse(str_detect(name,"control"),"standard", "restricted connectedness")) %>% 
  mutate(parameter=str_split_fixed(name,"_",3)[,2]) %>% 
