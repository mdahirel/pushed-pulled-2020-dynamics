...
plot_a <- expand_grid(dispersal.rate = c(0.05, 0.1, 0.2, 0.4), N = 1:500) %>%
  ...
  scale_x_log10(name = "", breaks = c(1, 2, 5, 10, 20, 50, 100, 200, 500), limits = c(0.4, 500)) +
...
It looks like this model does not reproduce the data very well. Maybe we should do like with genetic diversity and use a student model on the logits instead?
...
if (file.exists(here("R_output", "model3_popsize_expe.Rdata"))) {
  ...
  save(list = "mod_popsize", file = here("R_output", "model3bis_popsize_expe.Rdata"))
}
...
```{r}
posterior_samples(mod_K_v) %>%
  select(contains("cor_landscapeID__front_logspeedasym")) %>% 
  select(contains("logitPeggsest")) %>% 
  mutate(iter=1:dim(.)[1]) %>% 
  pivot_longer(cols=-iter) %>% 
  ggplot()+
  stat_eye(aes(x=name,y=value),point_interval=mean_hdi)+
  scale_y_continuous("correlation v_asym - K")


ttt=posterior_samples(mod_K_v) %>%
  select(contains("cor_landscapeID__front_logspeedasym")) %>% 
  select(contains("logitPeggsest")) 
...
###quick and dirty look at BLUP
as.data.frame(ranef(mod_K_v)$landscapeID[,"Estimate",]) %>% 
  rownames_to_column(var="landscapeID") %>% 
  mutate(is.pushed=substr(landscapeID,15,16)=="PS") %>% 
  ggplot()+
  geom_point(aes(logitPeggsest_Intercept,front_logspeedasym_Intercept))+
  facet_wrap(~is.pushed)
####oooooooo pretty
```
...
p1 <- data_genetics %>%
  ...
  ungroup() %>%
  ggplot() +
  stat_lineribbon(aes(x = Generation, y = .value, fill = Location), .width = c(0.66, 0.95), alpha = 0.4, point_interval = mean_hdi) +
...
p2 <- data_genetics %>%
  ...
  ungroup() %>%
  ggplot() +
  stat_eye(aes(x = Location, y = .value, fill = Location), slab_alpha = 0.9, point_interval = mean_hdi) +
  scale_fill_manual(values = edgecorePalette) +
  facet_wrap(~Treatment) +
