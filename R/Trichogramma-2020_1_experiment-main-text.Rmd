...
- find a valid color code !!! choose colors that are colorblind AND greyscale friendly (the front/core ones are currently NOT)
...
As a consequence, at the (relatively) low population sizes that interest us, reducing connectedness should lead expansions towards "pushed" dynamics, so two types of landscapes were set up: "reference" dispersal, with three tubes connecting successive patches, and "restricted" dispersal, with only one tube. They were hypothesized to lead to __relatively__ pulled and pushed dynamics, respectively. 12 landscapes were under putatively pushed dynamics, 12 under putatively pulled dynamics.
...
## as.numeric() resolves both correctly (e.g; "2." and "12" become 2 and 12)
  ...
  complete(nesting(Mix,Treatment,landscapeID),
                       Generation=0:13,fill=list(front=0)) %>% ##just so we have a G0 x0 point (i.e. release) for plotting
  ungroup()
...
We're going to do it in two steps: an intermediate dataset where we again extract useful variables from the `Image` string, and recode some other variables...
...
data_genetics<-raw_genetics %>% 
  ...
  mutate(Location=replace_na(Location,"origin")) %>% ## samples that are neither core nor front are from the origin (G0)
...
if(file.exists(here("R_output","model1_front_expe.Rdata")))
  # this if-else statement is avoid fitting a model when knitting Rmd file if there is already one existing in R_output
  # to override, best to first re-run and re-save manually the model by selecting relevant code lines then knit
  {
...
Ideally, we would have used a distribution actually accounting for the discrete nature of front locations values. One could try to fit a Poisson model instead of a lognormal one, but that would have been trading an approximation for another: data are **under**dispersed relative to the equivalent Poisson (you can change mod_front to Poisson and check this with pp_check()). A [Conway Maxwell Poisson]( https://discourse.mc-stan.org/t/brms-and-conway-maxwell-poisson-distribution/7368/15) may solve this, but it's not 100% implemented yet in brms, and may be overkill, so we stay with the lognormal approximation. Results are close to identical anyway
...
prior_genetics=c( ##only a part of the prior, so it can be used in several models ###the remainder in each brm() call
...
TRTpalette <- c("#F0E442","#CC79A7")
frontcorePalette <- c("#56B4E9","#E69F00","#000000") # 3 colors, for core, front and origin (black) patches
...
p1 <- data_front %>% 
  ...
  ggplot()+
  stat_lineribbon(aes(x=Generation,y=.value,fill=Treatment),.width=0.95,alpha=0.3,point_interval = mean_hdi)+
...
p1 <- data_genetics %>% 
  ...
  scale_fill_manual(values=frontcorePalette)+
