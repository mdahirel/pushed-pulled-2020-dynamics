...
But heterozygosity is always between 0 and 1. So why use a log-Student model, and not simply a Beta model?  
Well, we did both, and the former clearly outperforms the latter, maybe because it accounts better for rare outliers (which would be expected due to sampling when selecting individuals before genotyping, or due to the expansion process itself):

```{r model-genetics-beta, eval = FALSE}
bf_genetics2=bf(Hexp~log(H0*exp(-decay*Generation)),
               nlf(H0~inv_logit(logitH0)),
               nlf(decay~exp(logdecay)/10),
               logitH0~1+(1|1|landscapeID),
               logdecay~0+Treatment+Treatment:is.front+(is.front|1|landscapeID),
...
loo_student <- loo(mod_genetics)
loo_beta <- loo(mod_genetics2,reloo=TRUE)
loo_compare(loo_student, loo_beta) 
##despite Beta a priori a better spec, log-Student has better predictive performance 
##see https://mc-stan.org/bayesplot/reference/PPC-loo.html for further possibilities of investigation
#####

```

still, by forcing H0 to be between 0 and 1 using a logit transformation, we still ensure no "weird" values (e.g. > 1) are predicted by the log-Student model anyway
...
p1 / p2 + plot_annotation(tag_levels="A") + plot_layout(guides="collect") & theme_bw() &
  theme(legend.position="none")
