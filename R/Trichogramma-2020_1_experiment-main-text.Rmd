...
p2 <- data_genetics %>%
  ...
  scale_y_continuous(expression(paste("Mean genetic diversity decay rate  ", lambda)),
                     sec.axis = sec_axis(trans= ~0.5/., #if diploid, lambda = 1/(2*Ne) then Ne = 0.5*lambda
...
p2 <- cors_K_v %>% 
  ggplot()+
  stat_eye(aes(y=value,x=Treatment, fill= Treatment),point_interval=mean_hdi, slab_alpha = 0.9)+
