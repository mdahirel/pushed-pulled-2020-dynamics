...
At each generation, adults were left free to move and reproduce for 48 hours before being removed. These collected adults were then used as source of genetic information, and as sources of further offspring for another experiment. Patches were added to the end of each landscape as needed, so there was always at least two empty patches at the front. 
...
We first play with the dynamics dataset. From the raw dataset we want to produce a `data_front` dataset, to estimate how fast the expansions advance
...
Then we turn to the genetics data. We're going to convert individual microsatellite info into population/patch-level multilocus expected heterozygosity data, as our measure of genetic diversity.
...
Before we go forward, we also need to check whether a power decay in the non-linear formula wouldn't be more appropriate.
...
## but let's check their standardised performance:
...
## with prediction variance larger than data variance (see Gabry et al 10.1111/rssa.12378 and the bayesplot package doc)
## can also be seen using type="loo_pit_qq"
#####
```

Good. For other posterior checks we can't directly compare the Student model on logits and the Beta model (like information criterion-like ones).But to reassure ourselves we can compare the student with a Gaussian model on logits, which should be close to a Beta model in terms of behaviour:
...
It looks *bad*. We don't even need standardisation via LOO-PIT to see this model does not reproduce the data very well, at all. The pattern of "misfit" is the same as for the genetic data. Maybe we should do like with genetic diversity and use a student model on the logits instead?
...
It does look much better. Still not perfect, but better. And without an actual population dynamics model, we probably have nothing better to estimate the "extreme" values of the distribution accurately. So let's carry on with the Student model:
...
p2 <- data_front %>%
  ...
  #geom_jitter(data = subset(data_front, Generation == 13), 
  #           aes(x = Treatment, y = front/Generation, fill = Treatment), pch = 21, size = 2) +
  scale_fill_manual(values = TRTpalette) +
...
data_front %>%
  ungroup() %>%
  mutate(landscapeID = landscapeID[1], Generation = 1) %>% #the landscape variable is a dummy that is ignored; not needed for recent versions of brms
  select(Treatment, landscapeID, Generation) %>% 
  distinct() %>%
  add_fitted_draws(mod_front, nlpar = "logspeedasym", re_formula = NA) %>% #re_formula = NA is *why* the landscape variable is ignored
  mutate(.value = exp(.value)) %>% ## backtransformation
  compare_levels(variable = .value, by = Treatment) %>%
  mean_hdi() # no differences in asymptotic speed
```
...
p1 <- left_join(preds_v,preds_K) %>% 
  ggplot()+
  geom_segment(aes(x=lower_K,xend=upper_K,y=v,yend=v),col="grey85")+
  geom_segment(aes(x=K,xend=K,y=lower_v,yend=upper_v),col="grey85")+
  geom_point(aes(K,v,fill=Treatment),pch=21,size=2)+
  scale_y_continuous(expression(paste("predicted asymptotic velocity  ", italic(v))))+
  scale_x_continuous(expression(paste("predicted ", italic(K), " (mean % of hosts parasitized in starting patch)")))+
  scale_fill_manual(values=TRTpalette)+
  facet_wrap(~Treatment)+
  theme_half_open(11) +
  background_grid(colour.major = "grey95", colour.minor = "grey95")
