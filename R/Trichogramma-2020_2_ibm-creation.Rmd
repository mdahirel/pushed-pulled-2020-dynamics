...
# TO DO
parallelize the `running-simulations` chunk?
...
Now we need to add all of this (plus information on the variables that stay constant, which output variables will we save...) to the experiment slot of the NL object:
...
(Careful, script is not parallelized, so running 500 replicates in total is expected to take a few hours)
