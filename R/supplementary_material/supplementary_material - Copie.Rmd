---
title: "Supplementary Material for *Shifts from pulled to pushed range expansions caused by reductions in connectedness* (working title)"
author: "Maxime Dahirel, Aline Bertin, Marjorie Haond, Aurélie Blin, Eric Lombaert, Vincent Calcagno, Simon Fellous, Ludovic Mailleret, Elodie Vercken"
date:
output: 
  html_document:
    theme: yeti
...
<br/>
<br/>
**Note: ** The full code for the following document and the analyses it describes, as well as for all analyses described in the main text, will be made available on GitHub (**link to be added**) and Zenodo (**DOI to be added**).
...
data_G0_avg <- data_G0 %>% 
  ...
  ungroup() %>% 
  mutate(Treatment=fct_recode(factor(Treatment),reference="control"))
...
p1 <-newdata %>% 
  ...
  geom_point(data=data_G0 %>% mutate(Treatment=fct_recode(factor(Treatment),reference="control")),aes(x=jitteredTRT,y=distmean,col=Treatment),fill="white",pch=21,alpha=0.8)+
...
mod_front %>%
  ...
  mutate(treatment=ifelse(str_detect(name,"control"),"reference", name)) %>%
...
mod_front %>%
  ...
  mean_hdi() %>% 
  mutate(entry=paste(round(value,2)," [",round(.lower,2),", ",round(.upper,2),"]",sep="")) %>% 
  mutate(treatment=ifelse(str_detect(name,"control"),"reference", "restricted connectedness")) %>% 
  mutate(parameter=str_split_fixed(name,"_",3)[,2]) %>% 
...
mod_propa1=brm(bf(N_founders-1~0+treatment:K+(1|replicateID),
              ...
              prior=c(set_prior("normal(0,5)",class="b"),
                      set_prior("normal(0,1)",class="sd"),
                      set_prior("normal(0,1)",nlpar="invshape")),chains=4,iter=500,seed=42)
...
mod_propa2=brm(bf(N_founders/(N_source*0.5*disp0_mean)~0+treatment:K+(1|replicateID)),
              ...
              prior=c(set_prior("normal(0,1)",class="b"),
                      set_prior("normal(0,1)",class="sd"),
                      set_prior("normal(0,1)",class="sigma")),chains=4,iter=500,seed=42)
...
diffs_rel<-newdata %>% 
    add_fitted_draws(mod_popsize,re_formula=NA) %>% 
    ungroup() %>% filter(Treatment=="control") %>% left_join(diffs) %>% mutate(.value=.diff/.value) %>% mean_hdi(.value)
