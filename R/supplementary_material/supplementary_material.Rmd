...
**Figure S6.2** Observed (colored lines) and predicted (grey bands) among-replicate genetic variances as a function of $K$, time since start and treatment. The mean prediction curves are displayed along with their 95% credible bands.
...
The interested reader can get the same information for all other parameters by running the corresponding `Rmd` scripts (scripts 1 and 3), which contain various summary calls. Parameter names in the tables below are as in **Supplementary Material S5**.
...
# S8 - Model descriptions for models presented in Supplementary Materials

(As in S5, all Beta models described here use the (mean, precision) parametrisation of the Beta distribution.)

## S8.1 - validation of computer estimates of population sizes (Supplementary Material S2)
...
## S8.2 -  Validation of the experimental design (Supplementary Material S3)
