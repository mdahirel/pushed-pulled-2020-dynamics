---
title: "Supplementary Material for \"Shifts from pulled to pushed range expansions caused by reduction of landscape connectedness\" " 
...
<br/>
<br/>
**Note: ** The full code for the following document and the analyses it describes, as well as for all analyses described in the main text, is available on GitHub [here](https://github.com/mdahirel/pushed-pulled-2020-dynamics) and Zenodo [here](https://doi.org/10.5281/zenodo.3969988).
...
preds_obs <- newdata %>% add_fitted_draws(mod_validate,resp="Neggsobs",value="human",re_formula=~(1|spacetimeID)) %>% ungroup()

preds <- newdata %>% add_fitted_draws(mod_validate,resp="Peggsest",value="estimated",re_formula=~(1|spacetimeID)) %>% ungroup() %>% 
...
mod_front %>%
  ...
  #select(parameter,name,K,reference,`density-dependent dispersal`,`weak Allee effect`,`reduced connectedness`) %>%  #keep this select to check manually inputed names above are right
  select(name,reference,`density-dependent dispersal`,`weak Allee effect`,`reduced connectedness`) %>% #keep this select() for the true final version
...
mod_front %>%
  ...
  #select(parameter,name,standard,`reduced connectedness`) %>%  #keep this select to check manually inputed names above are right
  select(name,reference,`reduced connectedness`) %>% #keep this select() for the true final version
  column_to_rownames(var="name")  %>% 
  knitr::kable(format="html") %>%
  kable_styling()
```
...
mod_popsize <- brm(bf(Peggs_est~0+Treatment+(1|Macro)+(1|landscapeID), 
        nlf(phi~1/invphi),
        invphi~1),
...
$$
As in other Beta models, we used a $\mathrm{Normal}(0,1.5)$ prior for the treatment-specific intercepts $\beta_{0}$, and $\mathrm{Half\mbox{-}Normal}(0,1)$ priors for both the random effect standard deviation $\sigma_{\alpha}$ and $1/\phi$.
