...
$$
where $\mu_{x,i,t}$ is the mean genetic diversity, $\sigma_{d}$ the scale and $\nu$ the number of degrees of freedom. 
...
$$ 
P_{i,t,m} \sim \mathrm{Beta}(\mu_{i,m}, \phi) \\
\mathrm{logit}(\mu_{i,m}) = \beta_{\text{TREATMENT}[i]} +\alpha_{i} + \gamma_{m}\\
...
$$ 
\mathrm{logit}(P_{i,t,m}) \sim \mathrm{StudentT}(\mathrm{logit}(\mu_{i,m}), \sigma_{d} , \nu) \\
\mathrm{logit}(\mu_{i,m}) = \beta_{\text{TREATMENT}[i]} +\alpha_{i} + \gamma_{m}\\
\alpha_{i} \sim \mathrm{Normal}(0,\sigma_{\alpha}) \\
\gamma_{m} \sim \mathrm{Normal}(0,\sigma_{\gamma}) \\
$$
Priors are as in the previous model, so a $\mathrm{Normal}(0,1.5)$ for $\beta$s, and a $\mathrm{Half\mbox{-}Normal}(0,1)$ for random effect standard deviations $\sigma$. We again used a $\mathrm{Gamma}(2,0.1)$ prior for the Student-specific $\nu$.
