...
##to redo: models for popsize(push up both adapt_delta and tree_depth), and front simulation (main and ESM)
```
...
We found that released wasps did lay their eggs closer from release sites in landscapes with restricted connectedness compared to reference landscapes (**Supplementary Figure S3.1**).
...
mod_front %>%
  ...
  group_by(name) %>%
  mean_qi() %>%  ###TEMPORARY FIX TO AVOID MULIPART INTERVALS, FIX BACK TO QI SOON
  ...
  select(name,reference,`density-dependent dispersal`,`weak Allee effect`,`restricted connectedness`) %>% #keep this select() for the true final version
...
mod_front %>%
  ...
  select(name,reference,`restricted connectedness`) %>% #keep this select() for the true final version
  column_to_rownames(var="name")  %>% 
  knitr::kable(format="html") %>%
  kable_styling()
```
...
mod_front=brm(bf_front,
              data=subset(data_front,ticks>0),chains=2,iter=200,
