...
```
(NB: the fifth scenario is only used in Supplementary Material, but was simulated at the same time as the others so is in the same dataset)


This dataset contains the following variables (see script 2):
...
- `is.front`: whether the focal patch is the most advanced one at the time ("front", TRUE), or the initial, release patch ("core", FALSE). Combine with `ReplicateID`, define a time series (see `Location_full`)

-  `Hexp`: patch genetic diversiy (expected heterozygosity)

- `velocity_fisher`: expected velocity of a pulled wave with identical fecundity and dispersal at low density as the focal one (estimated here for population density = 1)
...
data_front <- data %>% 
  ...
  mutate(front=pxcor) %>% 
  mutate(K=factor(K)) %>% 
  filter(treatment!="reduced + DDD") ##supplementary material-only treatment, simulated at same time as others for convenience
...
The situation here is bit more complex. We can't simply use the hetezygosity per patch as in the experimental dataset, because there are many zeroes:
```{r data-genetics1}
#not shown
...
data_genetics <- data %>% 
  group_by(ticks,treatment,is.front,K) %>% # we group by "type" of patch and generation
...
Our model here is based on Gandhi et al 2019 PNAS and assumes genetic among replicate variance increases exponentially through time until reaching a plateau only determined by the initial allelic frequencies. The "decay" rate parameter is our parameter of interest here, and because we *know* the (average) initial allelic frequencies, we put a more informative prior on the plateau parameter:
...
prior_genetics=c(
  ...
  #note that are insensitive to changing that prior for a less informative one
...
###prediction intervals around each point
ppc_ribbon(yrep=predict(mod_genetics,summary=FALSE),
...
data_genetics %>% ungroup() %>% 
  ...
  ungroup() %>% 
  mutate(treatment=relevel(treatment,ref="reference")) %>% 
  ggplot()+
