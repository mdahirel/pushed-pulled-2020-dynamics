...
Fortunately, there is a way around this by using the among replicates variance in allelic frequencies rather than using heterozygosity directly (see Gandhi et al. 2019 PNAS)
```{r data-genetics2}
...
```{r data-genetics3}
# not shown
 ggplot(data_genetics) +geom_line(aes(x=ticks,y=varP1,group=is.edge,col=is.edge))+facet_wrap(~treatment+K)
### lots of zero, can't use
```
...
### this model is BIG, if you run into memory problem, a solution is to compute the log-likelihood for only part of the samples
  ...
  save(list = c("mod_front_IBM","loo_mod_front_IBM"), file = here("R_output", "model3_front_IBM.Rdata"))
}
```
...
if (file.exists(here("R_output", "model3bis_front_power_IBM.Rdata"))) {
  load(here("R_output", "model3bis_front_power_IBM.Rdata"))
} else {
...
mod_front_IBM2 <- brm(bf_front_IBM2,
  ...
  save(list = c("mod_front_IBM2","loo_mod_front_IBM2"), file = here("R_output", "model3bis_front_power_IBM.Rdata"))
}
...
mcmc_rank_overlay(mod_front_IBM, pars = summary_front$name)
```
