...
####we'll use method variance in fraction (gandhi 2019 pnas). works by "averaging across" replicates, no zeros in dataset
...
```{r}
if(file.exists(here("R_output","model3_genetics_IBM.Rdata")))
  {
...
save(list="mod_genetics",file=here("R_output","model3_genetics_IBM.Rdata"))
}
```


```{r}
if(file.exists(here("R_output","model4_front_IBM.Rdata")))
  {
...
mod_front=brm(bf_front,
              data=subset(data_front,ticks>0),chains=4,iter=12000,warmup=2000,
              prior=prior_front,control=list(adapt_delta=0.8,max_treedepth=10),
              seed=42)

save(list="mod_front",file=here("R_output","model4_front_IBM.Rdata"))
}
```
